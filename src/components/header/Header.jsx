...
const Header = () => {
  ...
  const handleSearch = (e) => {
    ...
    const isYoutubeSharedLink = searchQuery.includes("https://youtu.be");
    const isYoutubeURLLink = searchQuery.includes("https://www.youtube.com");

    if (isYoutubeSharedLink) {
      const parts = searchQuery.split(".be/");
      const videoId = parts[1].split("?")[0];
      return navigate(`/search/${videoId}`);
    } else if (isYoutubeURLLink) {
  ...
  const navLinks = [
    ...
    { title: "Trending", link: "/trending", icon: <IoTrendingUp size="100%" /> },
    { title: "Explore", link: "/explore", icon: <IoCompass size="100%" /> },
    { title: "Playlist", link: "/imported-playlist", icon: <PiPlaylist size="100%" /> },
  ...
  return (
    <>
      <header className="header-section">
        <div className="header-container container">
          <Link to="/" className="header-logo-wrapper">
            <img className="header-logo" src="/sb music logo.png" alt="SB-music" />
          </Link>
          <div
          ...
          <form
            className={`header-search-wrapper ${isSearchClick ? "mobile-search" : ""}`}
          ...
          {navLinks.map((item, index) =>
            item.link ? (
              <Link
                ...
                to={item.link}
                title={item.title}
                className={`bottom-nav-link absolute-center ${currentPath === item.link && !isMoreOptions ? "active-link" : ""}`}
              >
                {item.icon}
                <span className={`${currentPath === item.link && !isMoreOptions ? "bottom-nav-text" : "hide"}`}>
                  {item.title}
                </span>
              </Link>
            ) : (
              <button
                key={index}
                title={item.title}
                className={`bottom-nav-link more-btn absolute-center ${isMoreOptions ? "active-link" : ""}`}
                onClick={() => setIsMoreOptions(true)}
              >
                {item.icon}
                <span className={`${isMoreOptions ? "bottom-nav-text" : "hide"}`}>
                  {item.title}
                </span>
              </button>
            )
      ...
      {/* More options modal */}
      <div className={`more-options-container ${isMoreOptions ? "more-options-container-visible" : "more-options-container-hide"}`}>
        <div
          className="more-options-overlayer model-overlayer"
          onClick={() => setIsMoreOptions(false)}
        ></div>
        <div className={`more-options-model ${!isMoreOptions ? "more-options-model-inActive" : ""}`}>
      ...
      {/*  Always render Popup, control with isPopup prop */}
      <Popup
        isPopup={isPopup}
